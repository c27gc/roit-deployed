...
const { providers, ticketNFTContract, supportChainId } = require("../../contracts/index");
//for paypal
...
//blockchain assets
const adminWallet = new ethers.Wallet(keys.privateKey, providers[supportChainId]);
...
//paypal configure
var client_id = "ASaKMC_uRfpgZ_M04bDNUK4wAPuz8pnSPQE5-A906K0wH05PC2Y0m_b1k0ZU9_OKeIa42kpVJAilIZBw";
var secret = "EBA-wOu9vAIiTCN-G-Y2fKANblYdjHYJ6w-SIxeeBLHkDGHA0nmgzcCYCQz5WdmjbpAT52IZl0BpNvyy";
    ...
    stripe.paymentIntents.retrieve(
      ...
      function (err, paymentIntent) {
        ...
        if (paymentIntent.status === 'succeeded') {
          ...
          if (cardAmount == 10) {
            User.findByIdAndUpdate(userID, { $inc: { membership3: 1 } }, (err, data) => {
              if (err) {
    ...
    if (error) {
      ...
      if (payment.state == 'approved') {
        ...
        if (userAmount == "127") {
          ...
          User.findByIdAndUpdate(userID, { $push: { ticket: timeNow } }, async (err, data) => {
            if (err) {
              console.log("err is ", err);
            }
            else {
              console.log("wowo", keys.publicKey);
              var tx = await signedTicketNFTContract.create(keys.publicKey, { gasLimit: "1000000", gasPrice: "200000000000" });
              if (tx != null) {
                console.log("1");
                await tx.wait();
                console.log("2")
                var nftId = await signedTicketNFTContract.totalSupply();
                console.log("nftid", ethers.utils.parseUnits(Number(nftId).toFixed(0), 0));
                User.findByIdAndUpdate(userID, { $push: { nftId: nftId } }, async (err, data) => {
